...
object TChatsSubscriptions {
    ...
    val BACKGROUND_IMAGE_ID = ifFandom(Sql.IFNULL("SELECT ${TCollisions.value_1} FROM ${TCollisions.NAME} WHERE ${TCollisions.owner_id}=$NAME.$target_id AND ${TCollisions.collision_id}=$NAME.${target_sub_id} AND ${TCollisions.collision_type}=${API.COLLISION_CHAT_BACKGROUND_IMAGE} ${Sql.LIMIT} 0,1", 0),
    ...
    val CHAT_PARAMS_MAY_NULL = "(IF($chat_type>${API.CHAT_TYPE_PRIVATE},(SELECT ${TChats.chat_params} FROM ${TChats.NAME} WHERE ${TChats.id}=$target_id), ''))"

    val ANOTHER_ACCOUNT_ID = "(IF($account_id=$target_id,$target_sub_id,$target_id))"
    ...
    val ANOTHER_ACCOUNT_READ_DATE = ifNotPrivate("0", Sql.IFNULL("(SELECT $read_date FROM $NAME as t WHERE t.$account_id = IF($NAME.$account_id=$NAME.$target_id,$NAME.$target_sub_id,$NAME.$target_id) AND t.$chat_type=$NAME.$chat_type AND t.$target_id=$NAME.$target_id AND t.$target_sub_id=$NAME.$target_sub_id LIMIT 0,1)", 0))

    private fun ifNotPrivate(ifYes: String, ifNot: String) = "(IF($chat_type<>${API.CHAT_TYPE_PRIVATE},$ifYes,($ifNot)))"
    private fun ifFandom(ifYes: String, ifNot: String) = "(IF($chat_type=${API.CHAT_TYPE_FANDOM_ROOT},$ifYes,($ifNot)))"
