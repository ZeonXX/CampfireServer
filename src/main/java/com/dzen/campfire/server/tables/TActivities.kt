...
object TActivities {
    ...
    fun myPostId(accountId: Long) = Sql.IFNULL("(SELECT " + TActivitiesCollisions.tag_1 + " FROM " + TActivitiesCollisions.NAME + " WHERE " + TActivitiesCollisions.account_id + "=" + accountId + " AND " + TActivitiesCollisions.activity_id + "=" + TActivities.NAME + "." + TActivities.id + " AND " + TActivitiesCollisions.type + "=" + API.ACTIVITIES_COLLISION_TYPE_RELAY_RACE_POST + " LIMIT 0,1)", "0")
    fun myMemberStatus(accountId: Long) = Sql.IFNULL("(SELECT " + TActivitiesCollisions.tag_1 + " FROM " + TActivitiesCollisions.NAME + " WHERE " + TActivitiesCollisions.account_id + "=" + accountId + " AND " + TActivitiesCollisions.activity_id + "=" + TActivities.NAME + "." + TActivities.id + " AND " + TActivitiesCollisions.type + "=" + API.ACTIVITIES_COLLISION_TYPE_RELAY_RACE_MEMBER + " LIMIT 0,1)", "0")
