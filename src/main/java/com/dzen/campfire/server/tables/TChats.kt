...
object TChats {
    ...
    fun LAST_MESSAGE_ID(accountId:Long) = Sql.IFNULL("(SELECT ${TChatsSubscriptions.last_message_id} FROM ${TChatsSubscriptions.NAME} as t WHERE t.${TChatsSubscriptions.chat_type}=$NAME.$type AND t.${TChatsSubscriptions.target_id}=$NAME.$id AND t.${TChatsSubscriptions.account_id}=$accountId) ${Sql.LIMIT} 0,1)",  0)
    fun LAST_MESSAGE_ID_ANY_ACCOUNT() = Sql.IFNULL("(SELECT ${TChatsSubscriptions.last_message_id} FROM ${TChatsSubscriptions.NAME} as t WHERE t.${TChatsSubscriptions.chat_type}=$NAME.$type AND t.${TChatsSubscriptions.target_id}=$NAME.$id AND t.${TChatsSubscriptions.member_status}=${API.CHAT_MEMBER_STATUS_ACTIVE} ${Sql.LIMIT} 0,1)",  0)
