    ...
    private fun loadSearch(): Array<Fandom>{
        ...
        if (name.isNotEmpty()) {
            ...
            val v = Database.select("EFandomsGetAll.loadSearch",SqlQuerySelect(TCollisions.NAME, TCollisions.owner_id)
                    ...
                    .whereValue(SqlWhere.WhereLIKE(TCollisions.value_2), "%${Sql.mirror(name.toLowerCase())}%"))
            val ids = Array<Long>(v.rowsCount){v.next()}

            if(ids.isNotEmpty()) select.whereValue(SqlWhere.WhereString("(${SqlWhere.WhereLIKE(TFandoms.name).toQuery()} OR ${SqlWhere.WhereIN(TFandoms.id, ids).toQuery()})"), "%${Sql.mirror(name.toLowerCase())}%")
            else select.whereValue(SqlWhere.WhereLIKE(TFandoms.name), "%${Sql.mirror(name.toLowerCase())}%")
        }
